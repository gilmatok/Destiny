...
using Destiny.Maple.Instances.Implementation;
...
namespace Destiny.Maple.Life.Npcs
{
    public sealed class Npc9900000 : Npc
    {
        ...
        public override async Task Converse(Character talker)
        {
            ...
            if (test)
            {
                bool result = await this.ShowYesNoDialog(talker, "Woohoo! It's test time! Time to test the infamous #binstances#k. Would you like to give it a try now?~");

                if (result)
                {
                    var instance = new TestInstance(talker.Client.Channel);

                    instance.Start();

                    instance.AddCharacter(talker);

                    talker.ChangeMap(0);
                }
                else
                {
                    await this.ShowOkDialog(talker, "Well, alright. They're fun, but if you don't want to...");
                }
            }
            else
            {
